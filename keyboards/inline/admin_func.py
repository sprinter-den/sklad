...
def search_profile_func(user_id):
    ...
    user_purchases_kb = InlineKeyboardButton(text="📜 Его Запросы", callback_data=f"show_purchases:{user_id}")
    add_balance_kb = InlineKeyboardButton(text="🎊 Выдать бонус", callback_data=f"add_balance:{user_id}")
    set_balance_kb = InlineKeyboardButton(text="📦 Изменить наличие", callback_data=f"set_balance:{user_id}")
